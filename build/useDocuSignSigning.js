...
import { addSigningCancelledListener, addSigningCompleteListener, addSigningErrorListener, initialize, loginWithAccessToken, presentCaptiveSigning, presentCaptiveSigningWithUrl, } from './api';
export function useDocuSignSigning(options) {
    ...
    const startSigning = useCallback(async (session) => {
        try {
            if (!initializedRef.current) {
                await doInitialize();
            }
            ...
            if (session.type === 'url') {
                if (Platform.OS !== 'ios') {
                    throw new Error('presentCaptiveSigningWithUrl is iOS-only. Use the session flow (type: "session") for Android parity.');
                }
                setState('signing');
            ...
            setState(signingResult.status === 'completed'
                ? 'completed'
...
//# sourceMappingURL=useDocuSignSigning.js.map